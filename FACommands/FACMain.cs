...
{
    private readonly Dictionary<string, Player> _playerList = new ();
    private readonly Random _random = new (DateTime.Now.Millisecond);
    private Config _config = new ();

    public override string Name => "FACommands";
    public override Version Version => Assembly.GetExecutingAssembly().GetName().Version ?? throw new NullReferenceException("Failed to fetch assembly version");
    ...
    private void OnInitialize(EventArgs args)
    {
        ...
        Commands.ChatCommands.Add(new Command("facommands.staff", FacUserInfo, "uinfo")
        {
            HelpText = "Lists detailed information about players."
        });
        Commands.ChatCommands.Add(new Command("facommands.staff", FacBanInfo, "binfo")
        {
        ...
        {
            HelpText = "Roll a Dice! As a parameter you can set the range!",
            AllowServer = false
        });
    }
    ...
    private void FacSlay(CommandArgs args)
    {
        if (!FirstCheck(args, "slay", new []{"<player>", "<reason>"}, _config.SlayCooldown, c => c < 2))
            return;
        if (!FindPlayer(args.Parameters[0], args.Player, out var target) || target == null)
            return;
    ...
    {
        TShock.Log.ConsoleInfo("Kis command!");
    ...
    private static void FacUserInfo(CommandArgs args)
    {
        if (args.Parameters.Count == 1 && args.Parameters[0].Length != 0)
        {
            ...
            if (userByName != null)
            {
                ...
                try
                {
                    ...
                    args.Player.SendMessage($"{user}'s last known IP is {LastIP(userByName)}.", Color.DeepPink);
    ...
    private static void FacBanInfo(CommandArgs args) 
    {
        if (args.Parameters.Count == 0)
        {
            args.Player.SendErrorMessage("Invalid syntax: /binfo <full player username>");
            return;
        }
        var banByName = TShock.Bans.GetBansByIdentifiers(false, $"acc:{args.Parameters[0]}").LastOrDefault();
        if (banByName == null)
        {
            args.Player.SendErrorMessage("No bans by this identifier were found.");
            return;
        }
        var account = TShock.UserAccounts.GetUserAccountByName(args.Parameters[0]);
        args.Player.SendInfoMessage($"Account name: {account.Name} (Last IP: {LastIP(account)})");
        args.Player.SendInfoMessage($"Date banned: {banByName.BanDateTime} ({banByName.GetPrettyTimeSinceBanString()} ago)");
        var expires = banByName.GetPrettyExpirationString();
        args.Player.SendInfoMessage($"Expires in: {expires}" + (expires == "Never" ? "" : $" ({banByName.ExpirationDateTime})"));
        args.Player.SendInfoMessage($"Banning user: {banByName.BanningUser}");
        args.Player.SendInfoMessage($"Reason: {banByName.Reason}");
    }

    public static string LastIP(UserAccount account)
    {
        var stringList = JsonConvert.DeserializeObject<List<string>>(account.KnownIps) ?? new List<string>();
        return stringList.FirstOrDefault() ?? "Unknown";
    }

    private static bool FindPlayer(string param, TSPlayer issuer, out TSPlayer? player)
    {
        player = null;
